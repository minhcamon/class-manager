...
### 1.1 Mục đích
ClassManager là hệ thống quản lý lớp chủ nhiệm trực tuyến dành cho các trường học, giúp giáo viên chủ nhiệm theo dõi điểm thi đua, hồ sơ học sinh, và tổng kết tuần một cách tự động và minh bạch. Hệ thống hỗ trợ đa trường học, đa năm học và có khả năng mở rộng quy mô linh hoạt.

### 1.2 Phạm vi hệ thống
- **Quản lý đa trường (Multi-school):** Hệ thống có cấu trúc tổ chức từ Trường học ➔ Năm học ➔ Lớp học ➔ Tổ.
...
| Role | Mô tả |
|------|-------|
| **ADMIN** | Quản trị viên hệ thống — phê duyệt tài khoản Giáo viên (`TEACHER`), quản lý danh mục Trường học (`schools`). |
| **TEACHER** | Giáo viên chủ nhiệm — quản trị viên của lớp học được phân công và trực thuộc một Trường học. |
...
- **BR-AUTH-03:** Giáo viên tự đăng ký phải chọn trường học trực thuộc. Tài khoản Giáo viên chỉ được hoạt động sau khi được `ADMIN` phê duyệt.
...
- **BR-AUTH-05:** JWT Access Token có thời hạn 2 giờ, chứa thông tin `userId`, `role`, `schoolId`, `classId` (nếu có), và `groupId` (nếu có). Refresh Token có thời hạn 7 ngày, được lưu trữ trong HttpOnly Cookie bảo mật.
...
### 3.2 Nhóm Trường học & Năm học
- **BR-YEAR-01:** Mỗi trường học (`schools`) có danh sách năm học (`school_years`) độc lập. Chỉ cho phép tối đa 1 năm học ở trạng thái `ACTIVE` cho mỗi trường tại một thời điểm.
- **BR-YEAR-02:** Giáo viên chủ nhiệm có thể kết thúc năm học hiện tại của lớp/trường tương ứng (chuyển trạng thái `school_years.status` sang `ENDED`). Khi năm học đã `ENDED`, toàn bộ dữ liệu điểm số, log điểm, và báo cáo tuần của năm học đó trở thành chỉ đọc (read-only), không cho phép tạo mới hay thay đổi.
- **BR-YEAR-03:** Khi tạo năm học mới (`ACTIVE`), hệ thống tự động tăng khối lớp của học sinh lên 1 (ví dụ từ khối 10 sang khối 11). Học sinh đã hoàn thành khối 12 sẽ được tự động cập nhật trạng thái hồ sơ thành `GRADUATED`.
...
- **BR-POINT-01:** Điểm thi đua hiện tại của học sinh được tính động bằng: `Base Point` (của năm học đang hoạt động) cộng với tổng điểm (`point_value`) từ tất cả các bản ghi `point_logs` hợp lệ của học sinh đó trong năm học hiện tại.
...
- **BR-AUDIT-02:** Các hành động bắt buộc phải ghi log bao gồm:
  ...
  - Kết thúc năm học (End School Year).
...
-- 3. NĂM HỌC ĐỊA PHƯƠNG (SCHOOL YEARS)
CREATE TABLE school_years (
    id          SERIAL PRIMARY KEY,
    name        VARCHAR(20)  NOT NULL,                 -- Ví dụ: "2025-2026"
...
-- Ràng buộc: chỉ cho phép tối đa 1 năm học ACTIVE trên mỗi trường
CREATE UNIQUE INDEX uq_school_year_active_per_school
    ON school_years (school_id, status)
    WHERE status = 'ACTIVE';

-- 4. HỒ SƠ GIÁO VIÊN (TEACHER PROFILES)
...
CREATE TABLE classes (
    ...
    created_at     TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (class_name, school_id, school_year_id)
);

-- 6. HỒ SƠ HỌC SINH (STUDENT PROFILES)
...
-- 9. LOG ĐIỂM SỐ (POINT LOGS - IMMUTABLE)
...
CREATE INDEX idx_point_logs_student ON point_logs(student_id, school_year_id);
CREATE INDEX idx_point_logs_week ON point_logs(week_start_date);

-- 10. BÁO CÁO TUẦN (WEEKLY REPORTS)
...
CREATE INDEX idx_weekly_reports_student ON weekly_reports(student_id, school_year_id);
CREATE INDEX idx_weekly_reports_week ON weekly_reports(week_start_date, is_locked);

-- 11. FORM ĐỘNG (FORM TEMPLATES - VERSIONED)
...
-- 12. LOG GIÁM SÁT HỆ THỐNG (AUDIT LOGS)
...
### 5.3 Quản lý Năm học (School Years)

#### POST `/api/v1/school-years`
Khởi tạo năm học mới của một trường.
- **Phân quyền:** `TEACHER` (quản trị trường/lớp tương ứng).
- **Body:** `{"name": "2025-2026", "basePoint": 100, "schoolId": 1}`
- **Response 210:** Trả về đối tượng `SchoolYear` mới tạo. Ghi nhận Audit Log.

#### PUT `/api/v1/school-years/{id}/end`
Kết thúc năm học hiện tại. Toàn bộ dữ liệu năm học chuyển sang read-only.
- **Phân quyền:** `TEACHER`.
- **Response 200:** Trả về `SchoolYear` có `status = ENDED`. Ghi nhận Audit Log.
...
- `SCHOOL_YEAR_ENDED` (409): Năm học đã kết thúc, dữ liệu ở trạng thái read-only.
- `ACTIVE_SCHOOL_YEAR_EXISTS` (409): Đã có một năm học ở trạng thái ACTIVE trong trường này.
...
### 9.1 Tính mở rộng và Tách biệt dữ liệu (Multi-tenancy)
- Toàn bộ dữ liệu của Trường học, Năm học, Lớp học phải được liên kết chặt chẽ qua khóa ngoại nhằm tránh rò rỉ dữ liệu chéo giữa các trường học khác nhau.
- Chỉ số index được thiết lập trên tất cả các khóa ngoại (`school_id`, `class_id`, `user_id`, `student_id`) để tối ưu hóa truy vấn khi lượng dữ liệu lớn.
...
public void weeklyLockJob() {
    // 1. Quét toàn bộ các năm học đang ACTIVE của tất cả các trường
    // 2. Với mỗi lớp học trong năm học ACTIVE:
...
- [ ] Triển khai thực thể `User`, `TeacherProfile`, `StudentProfile` và `School`.
...
- [ ] Xây dựng API Năm học (`school_years`) địa phương hóa theo Trường.
...
│   ├── SchoolYearController.java      -- Quản lý năm học
